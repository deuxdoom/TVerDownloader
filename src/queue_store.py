...
**롤링 백업을 두지 않는 것이 HistoryStore와 다른 점이다.** 이쪽은 대기열이 바뀔 때마다 다시
쓰여서, 50개를 넣는 한 번의 조작만으로 백업이 50벌 쌓이고 그중 어느 것도 다시 볼 일이 없다.

**쓰기는 동기다.** 마지막 한 번이 앱을 끝내기 직전(stop_all)에 일어나 다른 스레드에 맡기면
그 쓰기가 끝나기 전에 프로세스가 사라진다.
"""
...
class QueueStore:
    ...
    def _clean(raw: Any) -> List[Dict[str, str]]:
        ...
        **차례가 곧 대기열 차례라 정렬하지 않는다.** 같은 주소가 두 번 들어 있으면 앞의
        것만 남긴다 - 카드는 주소당 하나뿐이고, 대기열에 둘이면 그 항목만 두 번 받으러 간다.
        """
    ...
    def replace(self, items: Iterable[Dict[str, str]]) -> None:
        ...
        덧붙이지 않는 것은 빠진 것을 따로 지워 달라고 하면, 지우는 자리를 하나라도 빠뜨렸을
        때 이미 받은 항목이 다음 실행에 되살아나기 때문이다.
        """
    ...
    def save(self) -> bool:
        ...
        임시 파일에 썼다가 바꿔치기한다. 앱을 끝내는 길목에서 쓰이므로 쓰는 도중에 프로세스가
        사라질 수 있고, 반쯤 쓰인 파일이 남으면 다음 실행에서 대기열을 통째로 못 읽는다.
        """
        target = Path(self.path)
