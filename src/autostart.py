...
HKCU의 Run 키에 값 하나만 쓴다 - 관리자 권한이 필요 없고 끄면 흔적이 남지 않는다(시작
프로그램 폴더의 바로 가기는 파일을 남기고 사용자가 지워도 앱이 알 수 없다). 로그인할 때
뜨는 것이라 창을 띄우지 않도록 TRAY_FLAG를 붙여 등록한다.
"""
...
def target_command() -> str | None:
    ...
    소스로 등록해 두면 개발 중인 사본이 로그인할 때마다 뜬다.
    """
...
def is_enabled() -> bool:
    ...
    값이 있어도 가리키는 경로가 다르면 꺼진 것으로 본다 - 폴더를 옮겼는데 켜져 있다고
    보여 주면 사용자는 이 exe가 자동 실행된다고 믿는다. 다시 켜면 지금 경로로 갱신된다.
    """
...
def set_enabled(enabled: bool) -> bool:
    ...
    끌 때 값이 원래 없어도 성공으로 본다 - 바란 상태가 이미 그대로라 알릴 것이 없다.
    """
