...
- **다국어를 지원합니다.** OS의 주 언어에 따라 해당 언어로 나오고, 설정에서 직접 고르면 다시 시작하지 않아도 곧바로 바뀝니다.
...
- **끌어다 놓기** — 브라우저 주소창에서 창으로 직접. 여러 개를 놓으면 다중 추가 창이 채워진 채 열림. 크롬처럼 `https://`를 떼고 넘겨주는 브라우저에서 끌어와도 TVer 주소로 알아봄
...
- 최신 **yt-dlp** 및 **FFmpeg** 자동 다운로드 및 업데이트 — 로그 옆 **`도구 업데이트`** 단추로 언제든 다시 확인할 수 있음. 켤 때 인터넷이 안 되면 기다리지 않고 설치된 것으로 바로 시작하고, 로그에는 이유만 한 줄로 남김
...
- **프로그램 안에서 바로 교체** — 새 버전 안내창의 `지금 업데이트`를 누르면 확인 → 받기 → 적용 → 완료를 단계별로 보여 주며 알아서 교체함. 브라우저에서 zip을 받아 덮어쓸 필요 없음
- **설정 · 기록 · 즐겨찾기는 그대로 유지** — 교체 대상은 실행 파일과 `_internal` 폴더뿐
- **`변경 내역`** — 받기 전에 무엇이 바뀌었는지 먼저 확인
- **원할 때 직접 확인** — 정보 창의 `업데이트 확인`. 자동 확인은 설정 > 일반에서 끌 수 있음
- **교체에 실패하면 이전 버전으로 되돌림** — 백업은 다음 실행 때 자동으로 정리됨

### 파일 및 변환

- **파일명 구성 요소 커스터마이징** — 항목을 **끌어놓아** 순서를 바꾸고, 실제 방송 예시로 결과를 미리 확인. 구성 요소마다 **색이 붙어** 어느 것이 파일명의 어디가 되는지 한눈에 보이고, 체크를 풀면 그 줄이 흐려짐. 최소 한 항목을 선택해야 저장할 수 있으며, 기존 설정이나 영상 정보 때문에 이름이 비면 영상 ID로 구분함
...
- **창 폭과 언어에 맞는 배치** — 도구 줄이 자동으로 줄을 나누고, 좁은 창에서는 로그가 다운로드 목록 아래로 이동. 로그를 접어도 중요한 안내는 목록 위에서 확인하고 눌러 로그를 열 수 있음
- **읽기 편한 두 테마** — 라이트의 짙은 파란 주 버튼과 다크의 네이비 배경, 또렷한 카드 제목. 넓은 창에서는 가장자리 여백이 늘어남
- **작은 화면에 맞는 설정 창** — 본문만 스크롤하고 저장·나가기 단추는 고정. 긴 체크 항목도 줄을 나누어 표시
...
- **지역 확인** (항상 켜짐) — 준비가 끝나면 로그 `안내` 칸에 실제 확인 결과. 일본이면 초록빛으로 `현재 일본 IP 입니다`, 아니면 붉은색으로 `현재 일본 IP가 아닙니다 / OO 국가이므로 VPN을 켜주세요`. 확인하지 못했으면 그 사실을 밝히고 일반 안내를 보여 줌(실패를 '일본 아님'으로 말하지 않음). 알려 줄 뿐 다운로드를 막거나 창을 띄우지 않고, IP 주소는 로그에 적지 않음
- **VPN을 나중에 켜도 안내가 따라옴** — 네트워크가 바뀌는 것을 알아채고 그때 다시 확인하므로, 프로그램을 껐다 켤 필요 없이 잠시 뒤 새 상태로 바뀜. 받는 중에 VPN이 끊긴 것도 같은 방식으로 알려 줌. **평소에는 아무것도 확인하지 않음** — 몇 초마다 되풀이해 묻는 방식이 아니라 윈도우가 알려 주는 네트워크 변경을 받아 그 순간에만 확인. 나라가 실제로 달라졌을 때만 새 안내가 붙고, VPN을 켜고 끄는 사이의 일시적인 확인 실패는 조용히 넘김
- **로그를 접어 두어도 바뀌는 것이 보임** — 목록 위 한 줄 안내의 빨간 경고가 VPN을 켜면 초록으로 바뀌고 10초 뒤 사라져 목록에 자리를 돌려줌. 처음부터 일본 IP면 아무것도 뜨지 않음
...
- **썸네일 캐시 관리** 기능
- **한국어 · 일본어 · 영어 · 중국어(간체 · 번체) · 스페인어 · 태국어 지원** (설정 > 일반, 기본값은 윈도우 표시 언어 자동 감지) — 버튼 · 메뉴 · 탭 · 설정 · 로그 패널과 업데이트 진행 창까지 고른 언어로 나옴. 입력칸을 우클릭하면 나오는 `잘라내기` · `복사` · `붙여넣기`처럼 Qt가 직접 만드는 메뉴도 함께 바뀜(태국어만 Qt에 번역이 없어 그 자리가 영어로 남음). 중국어는 대만 · 홍콩 · 마카오에서 번체가, 본토 · 싱가포르에서 간체가 자동으로 잡히고, 번체는 대만에서 쓰는 낱말로 옮김. 헤더 로고 그림은 한국어 · 일본어 · 영어 세 가지뿐이고 나머지 언어는 영어 로고를 씀. 언어를 바꾸면 **다시 시작하지 않고 그 자리에서 바로 적용됨**(받는 중인 다운로드도 끊기지 않음). 번역은 `_internal/lang` 폴더의 ini 파일이라 직접 고치거나 **새 언어를 ini 하나만 넣어 추가할 수 있음**(그 언어 윈도우에서는 자동 감지로도 잡힘). 고친 것을 업데이트 뒤에도 남기려면 **exe 옆에 `lang` 폴더를 만들어 사본을 넣으면 됨** — 그쪽이 있으면 먼저 읽고, 빠진 항목만 새 번역으로 채워짐. **만드는 방법은 exe 옆 `도움말(README).txt`에 한국어 · 영어로 정리해 두었음**(파일명 규칙 · `[meta]`의 `code`/`display_name`/`author` · 자리표시자와 줄 수 규칙)
...
│  ├─ 📄 single_instance.py                  → 중복 실행 막기 (이름 있는 뮤텍스) · 두 번째 실행의 요청을 먼저 뜬 쪽에 전달
...
│  ├─ 📄 self_update.py                      → 업데이트 꾸러미 검증 · 작업 폴더 · 적용 실행 · 대체 배치
│  ├─ 📄 apply_update.py                     → 새 실행본에서 파일 교체 · 진행 알림 · 실패 시 되돌리기
...
│  ├─ 📄 thumbnails.py                       → 썸네일 스레드 풀(같은 그림은 한 번만 받음) + 디스크 캐시(목록용 사본 · 원본) + 메모리 캐시 (위젯 아님)
│  ├─ 📄 qtparts.py                          → 공용 Qt 부품 — 팝업 모양 · 둥근 메뉴 · 2열 그리드 · 휠 스크롤
│  ├─ 📄 window_frame.py                     → 제목 표시줄 없는 창 — 둥근 모서리 · 그림자 · 끌기 · 크기 조절 · 최대화 · 창 자리잡기
...
│  ├─ 📄 icons_data.py                       → 임베드된 SVG 25종 (자동 생성)
│  ├─ 📄 indicators.py                       → 체크·라디오·스피너 화살표 이미지 생성
│  ├─ 📄 titlelogo.py                        → 언어·테마별 헤더 로고 로드
│  ├─ 📄 net_watch.py                        → 네트워크가 바뀔 때만 알림 (VPN을 켜면 지역 확인을 다시 부름)
...
│  │  ├─ 📄 region_thread.py                 → 현재 IP의 국가 확인 — 켤 때와 네트워크가 바뀔 때 (IP는 남기지 않음 · 실패하면 조용히)
...
│     ├─ 📄 update_dialog.py                 → 업데이트 확인 · 받기 · 검증 · 압축 해제 안내
│     ├─ 📄 apply_update_window.py           → 본체 종료 후 교체 진행 · 완료 · 실패 결과 창
│     └─ 📄 update_flow.py                   → 업데이트 단계와 작업 목록의 공통 화면
├─ 📂 assets
...
├─ 📄 readme.txt                              → 배포본 안내 (한국어 · 영어) · 언어 파일 만드는 법 (저장소에는 없음, 배포본 전용)
...
│  ├─ 📂 thumbnails/                          → 썸네일 캐시 (`cards/` 목록용 작은 사본 · `originals/` 저장용 원본, 원본은 30일 뒤 자동 삭제)
