...
- **기록 탭에서 오른쪽 클릭** — 브라우저에서 열기 · 다시 다운로드 · 파일이 위치한 폴더 열기(파일이 남아 있을 때만) · 기록에서 제거
...
- **시작할 때 즐겨찾기 확인** (기본 꺼짐, 설정 > 일반) — TVer는 일본 지역 제한이 있어 VPN보다 프로그램이 먼저 뜨면 확인이 전부 실패로 끝나므로 꺼 두었음. 평소에는 즐겨찾기 탭의 `갱신`으로 원할 때만 확인
...
- **버튼 색으로 성격 구분** — 마우스를 올리면 되돌릴 수 없는 것(선택 항목 취소 · 즐겨찾기 삭제 · 기록 삭제)은 진한 빨강, 목록만 정리하는 것(완료 항목 삭제)은 주황, 즐겨찾기 `추가`는 파랑, `갱신`은 초록
