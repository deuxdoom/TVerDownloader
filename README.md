...
| 항목 | 내용 |
|---|---|
| 일반 | 저장 폴더, 동시 다운로드 수, 닫기 버튼(X) 동작, 클립보드 자동 인식, 시작 시 즐겨찾기 확인 |
...
- **업데이트는 프로그램 안에서 하는 것이 안전합니다.** 새 버전이 나오면 뜨는 안내창의 `지금 업데이트`를 누르면 알아서 교체되고, 설정 · 기록 · 즐겨찾기는 그대로 남습니다. **직접 덮어쓸 때는** 반드시 `TVerDownloader.exe` **파일**과 `_internal` **폴더**를 **함께** 바꿔야 합니다. 둘은 한 벌이라 따로 바꾸면 안 됩니다.
...
- **설정 저장**: JSON 기반(config / history / favorites) — 단축키 조합도 `downloader_config.json`에 함께 보관. `윈도우 시작 시 실행`만 레지스트리(`HKCU\...\Run`)에 값 하나로 기록되며, 끄면 지워집니다
...
│  └─ 🖼️ tver.ico                             → exe 아이콘
├─ 📂 tests                                    → pytest 로직 검사 (QApplication 없이 초 단위)
│  ├─ 📄 conftest.py                          → 가짜 스레드 주입 (진짜 yt-dlp를 띄우지 않는다)
│  └─ 📄 test_*.py                            → 주소 판별 · 파일명 · 대기열 · 진행률 · 재시도 · 업데이트 등
├─ 📂 tools
│  ├─ 📄 _bootstrap.py                       → sys.path·작업 디렉터리·UTF-8 출력 준비
...
│  ├─ 📄 make_update_test_build.ps1          → 버전을 낮춘 시험 빌드 (자동 업데이트 실검증용)
│  └─ 📄 test_*.py                            → 화면 회귀 검증 (단축키 · 클립보드 · 트레이 · 빈 목록 등)
├─ 🧾 실행 중 생성되는 항목
...
├─ 📄 .gitignore
├─ 📄 pytest.ini                              → pytest 수집 범위 (tests/ 만)
├─ 📄 requirements.txt                        → 실행에 필요한 것 (PyQt6, requests)
├─ 📄 requirements-dev.txt                    → 위 + pytest · pyinstaller
├─ 📄 README.md
├─ 📄 CHANGELOG.md                            → 버전별 변경 내역
├─ 📄 CLAUDE.md                               → 작업 지침 (규칙 · 실측 기록)
├─ 🖼️ logo.png
└─ 🖼️ main.png
```
...
- ❌ **무단 수정 및 가공 금지**: 원본 소스 코드를 임의로 수정, 변조하여 새로운 프로그램인 것처럼 위장하거나 파생 워크(Fork 등)를 만들어 배포하는 행위를 엄격히 금지합니다.
...
- ✅ **개인적 사용 및 학습 허용**: 개인적인 목적의 다운로드, 소스 코드 열람, 개인 환경에서의 로컬 빌드 및 사용은 자유롭게 허용됩니다.
