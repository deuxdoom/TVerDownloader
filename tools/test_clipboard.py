...
def teardown(*windows):
    """app이 살아 있는 동안 창을 거둔다(test_log.teardown과 같은 이유).
...
class ProbeInput(InputSources):
    """진짜 다중 추가 창은 exec()로 멈춰 서므로, 무엇을 들고 불렸는지만 받아 둔다."""

    def open_bulk_add(self, initial_urls=None):
        self.window.bulk_calls.append(list(initial_urls or []))
        return self.window.bulk_opens
...
report("기본은 연결 안 됨", host.input_sources._clipboard_connected is False)
clip.setText(EP); settle()
report("꺼진 상태에서는 입력창이 그대로", host.ui.url_input.text() == "",
       f"입력창={host.ui.url_input.text()!r}")

host.input_sources.apply_clipboard_watch(True)
report("켜면 연결됨", host.input_sources._clipboard_connected is True)

clip.setText(""); settle()
host.ui.url_input.clear(); host.input_sources._last_clipboard_url = ""
clip.setText(EP); settle()
...
host.input_sources._last_clipboard_url = ""
clip.setText(EP); settle()
...
host.input_sources.apply_clipboard_watch(False)
report("연결 해제됨", host.input_sources._clipboard_connected is False)

probe = {"hits": 0}
original = host.input_sources.on_clipboard_changed
...
host.input_sources.on_clipboard_changed = counting
host.ui.url_input.clear(); host.input_sources._last_clipboard_url = ""
...
host.input_sources.on_clipboard_changed = original
...
for _ in range(3):
    ...
    host.input_sources.apply_clipboard_watch(False)
    host.input_sources.apply_clipboard_watch(False)
report("중복 호출에도 상태가 어긋나지 않음", host.input_sources._clipboard_connected is False)
host.input_sources.apply_clipboard_watch(True)
host.ui.url_input.clear(); host.input_sources._last_clipboard_url = ""
...
def reset(input_text=""):
    ...
    host.input_sources._last_clipboard_url = ""
    host.input_sources._bulk_dialog = None
...
host.input_sources._bulk_dialog = dialog
