...
class MainWindow(QMainWindow):
    ...
    def refresh_history_list(self):
        search_term = self.ui.history_search_input.text().lower()
        sort_index = self.ui.history_sort_combo.currentIndex()
        ...
        for url, meta in entries_to_show:
            ...
            if meta.get("series_id") or meta.get("thumbnail_url"):
                widget = HistoryItemWidget(url, meta)
                item.setSizeHint(widget.sizeHint())
                self.ui.history_list.addItem(item)
                self.ui.history_list.setItemWidget(item, widget)
            else:
    ...
    def _on_series_parsed(self, context: str, series_url: str, episode_info: List[Dict[str, str]]):
        if context == 'single' or context == 'bulk':
            ...
            if not episode_info:
                self.append_log(f"[{context}] '{series_url}' 시리즈에서 에피소드를 찾지 못했습니다.")
                return
            dialog = SeriesSelectionDialog(episode_info, self)
            if dialog.exec():
                selected_urls = dialog.get_selected_urls()
                if not selected_urls:
                    self.append_log(f"[{context}] 선택된 에피소드가 없어 추가하지 않았습니다.")
                    return
                added_count = 0
                for url in selected_urls:
                    if self._request_add_task(url):
                        added_count += 1
                self.append_log(f"[{context}] 시리즈에서 선택한 {added_count}개 에피소드를 추가했습니다.")
            else:
                self.append_log(f"[{context}] 시리즈 에피소드 추가를 취소했습니다.")
            ...
            for episode in episode_info:
                ...
                if self._request_add_task(url):
                    added_count += 1
            if added_count > 0:
                self.append_log(f"'{series_url}'에서 신규 에피소드 {added_count}개를 추가했습니다.")
    ...
    def open_settings(self):
        ...
        if dialog.exec():
            ...
            if new_theme != current_theme:
                ...
                self.append_log(f"테마가 '{new_theme}' (으)로 변경되었습니다.")
